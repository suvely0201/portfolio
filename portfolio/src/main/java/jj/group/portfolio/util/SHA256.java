...
public class SHA256 {
	...
	private static final int ROTL_ULONG(int x, int n) { return (x << n) | Common.URShift(x, 32 - n); }
	private static final int ROTR_ULONG(int x, int n) { return Common.URShift(x, n) | (x << (32 - (n))); }
	private static final int ENDIAN_REVERSE_ULONG(int dwS) { return ( (ROTL_ULONG((dwS),  8) & 0x00ff00ff) | (ROTL_ULONG((dwS), 24) & 0xff00ff00) ); }
	private static final void BIG_D2B(int D, byte[] B, int B_offset) { Common.int_to_byte_unit(B, B_offset, D, ENDIAN); }
	...
	public String getSHA256Type(String inputStr){
		...
		try {
			...
			byte[] pbData = inputStr.getBytes("UTF-8");
			SHA256_Encrpyt(pbData, pbData.length, pbCipher);
			StringBuffer stringbuffer = new StringBuffer();
			
		    for (int i=0; i<32; i++){
		    	stringbuffer.append(Integer.toString((pbCipher[i]&0xff) + 0x100, 16).substring(1));
		    }
